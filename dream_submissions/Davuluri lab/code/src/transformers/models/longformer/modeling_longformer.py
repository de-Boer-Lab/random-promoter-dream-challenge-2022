...
def _get_question_end_index(input_ids, sep_token_id):
    ...
    assert (
        sep_token_indices.shape[0] == num_sep * batch_size
    ...
    return sep_token_indices.view(batch_size, num_sep, 2)[:, 0, 1]
...
class LongformerEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # initialize positional embedding with old ones
        # logger.info("Initializing positional embeddings from older DNABERT")
        # pretrained_pos_embedding = np.load("/home/zhihan/transformers/examples/dna/old_model/6-new-12w-0/position.npy")
        # pretrained_pos_embedding = torch.Tensor(pretrained_pos_embedding)
        # copied_pos_embedding = torch.zeros([config.max_position_embeddings, config.hidden_size])
        # pretrained_length = pretrained_pos_embedding.shape[0]
        # for i in range(config.max_position_embeddings//pretrained_length):
        #     copied_pos_embedding[2+i*pretrained_length:2+(i+1)*pretrained_length] = pretrained_pos_embedding
        # self.position_embeddings.weight = nn.Parameter(copied_pos_embedding)


        # # initialize word embedding with old ones
        # logger.info("Initializing word embeddings from older DNABERT")
        # pretrained_word_embedding = np.load("/home/zhihan/transformers/examples/dna/old_model/6-new-12w-0/word.npy")
        # pretrained_word_embedding = torch.Tensor(pretrained_word_embedding)
        # self.word_embeddings.weight = nn.Parameter(pretrained_word_embedding)

        # # initialize token type embedding with old ones
        # logger.info("Initializing token type embeddings from older DNABERT")
        # pretrained_type_embedding = np.load("/home/zhihan/transformers/examples/dna/old_model/6-new-12w-0/type.npy")
        # pretrained_type_embedding = torch.Tensor(pretrained_type_embedding)
        # self.token_type_embeddings.weight = nn.Parameter(pretrained_type_embedding)

        # # initialize embedding layer normolization with old ones
        # logger.info("Initializing embeddings layer normolization from older DNABERT")
        # pretrained_LNW_embedding = np.load("/home/zhihan/transformers/examples/dna/old_model/6-new-12w-0/LNW.npy")
        # pretrained_LNB_embedding = np.load("/home/zhihan/transformers/examples/dna/old_model/6-new-12w-0/LNB.npy")
        # pretrained_LNW_embedding = torch.Tensor(pretrained_LNW_embedding)
        # pretrained_LNB_embedding = torch.Tensor(pretrained_LNB_embedding)
        # self.LayerNorm.weight = nn.Parameter(pretrained_LNW_embedding)
        # self.LayerNorm.bias = nn.Parameter(pretrained_LNB_embedding)
...
class LongformerForSequenceClassification(LongformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.longformer = LongformerModel(config, add_pooling_layer=False)
        if False:
            self.classifier = LongformerClassificationAverageHead(config)
            print("Using average of token embeddings for sequence classification")
        else:
...
class LongformerClassificationAverageHead(nn.Module):
    """Head for sentence-level classification tasks."""
    ...
    def forward(self, hidden_states, attention_mask, **kwargs):
        hidden_states[torch.where(attention_mask==0)] = 0 
