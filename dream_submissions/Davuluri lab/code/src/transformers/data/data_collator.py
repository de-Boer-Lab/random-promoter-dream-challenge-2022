...
def default_data_collator(features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
    ...
    if "label" in first and first["label"] is not None:
        ...
        batch["labels"] = torch.tensor([f["label"] if f["label"]!=None else 10 for f in features], dtype=dtype)
...
class DataCollatorForLanguageModeling:
    ...
    def __call__(
        ...
        if self.mlm:
            if self.continue_mask:
                batch["input_ids"], batch["labels"] = self.continue_mask_tokens(
    ...
    #### continue mask
    def continue_mask_tokens(
        self, inputs: torch.Tensor, special_tokens_mask: Optional[torch.Tensor] = None
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
        MASK_LIST=[-3,-2,-1,0,1,2,3]
...
@dataclass
class DataCollatorForSameSentence(DataCollatorForLanguageModeling):
    def __call__(self, examples: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        same_sentence_label_list = [example["labels"] for example in examples]
        same_sentence_label = torch.stack(same_sentence_label_list)

        return {
            "input_ids": input_ids,
            "token_type_ids": token_type_ids,
            "labels": same_sentence_label,
        }
