...
class SeqDatasetProb(Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        """
        seq = self.transform(self.ds.Sequence.values[i]) # type: ignore
        to_concat = [seq]
        ...
        dev_bin = self.ds.Dev_log2_enrichment.values[i]
        hk_bin = self.ds.Hk_log2_enrichment.values[i]
        bins  = [dev_bin, hk_bin]

        return {"x": X.float(), 
                "y": np.asarray(bins, dtype=np.float32)# type: ignore
                }
    
    def __len__(self) -> int:
        return len(self.ds.Sequence)